...
proc accQuote(id: PNode): Pnode = nkAccQuoted.newTree(id)

proc dotExpr(a, b: PNode): Pnode = nkDotExpr.newTree(a, b)
...
proc exported(n: PNode): PNode = nkPostFix.newTree(star, n)
...
for face in doc.findall("interface"):
  ...
  block:
    let ty = nkObjectTy.newTree(
        newEmpty(),
        inheritWlObject,
        newNode(nkRecList)
      )
    # let faceVer = face.attr("version")
    let def = nkTypeDef.newTree(
        nkPostFix.newNode.add(star, faceTypeId),
        newEmpty(),
        nkRefTy.newTree(ty),
      )
    typeSection.add(def)
  procList.add nkFuncDef.newTree(
      "face".ident.exported,
      newEmpty(),
      newEmpty(),
    ...
    if subnode.kind == xnElement:
      ...
      of "enum":
        ...
        typeSection.add nkTypeDef.newTree(
            ident(facePrefix & "_" & subnodeName).exported,
            newEmpty(),
            enumTy,
          )
      of "request", "event":
        let
          ...
          procArgs = nkFormalParams.newTree(
              newEmpty(),
              objParam,
            )
        ...
        if subnode.tag == "event":
          ...
          if argsTuple.len > 0:
            ofStmts.add nkVarSection.newTree(
                nkIdentDefs.newTree(
                    argsId, argsTuple, newEmpty()
                  )
              )
            ofStmts.add nkCall.newTree(
                ident"unmarshal",
                objId,
                msgId,
                argsId,
          ...
          procList.add nkMethodDef.newTree(
              exportId,
              newEmpty(),
              newEmpty(),
              procArgs,
          ...
          procList.add nkProcDef.newTree(
              exportId,
              newEmpty(),
              newEmpty(),
              procArgs,
  ...
  if eventCaseStmt.len > 1:
    ...
    eventCaseStmt.add nkElse.newTree(
        nkStmtList.newTree(
            nkRaiseStmt.newTree(
                nkCall.newTree(
                    ident"newUnknownEventError",
                    faceName.newLit(),
    ...
    procList.add nkMethodDef.newTree(
        nkPostFix.newNode.add(star, ident"dispatchEvent"),
        newEmpty(),
        newEmpty(),
        nkFormalParams.newTree(
            newEmpty(),
...
let moduleText = renderTree(module, {renderIds, renderSyms, renderIr, renderNonExportedFields, renderExpandUsing, renderDocComments})
