...
for face in doc.findall("interface"):
  ...
  block:
    ...
    if enumTy.len > 1:
      typeSection.add(
        nkTypeDef.newTree(
          nkPostFix.newNode.add(star, ident(faceName & "_opcode")),
          newEmpty(),
          enumTy))
  ...
  let def = nkTypeDef.newTree(
      nkPostFix.newNode.add(star, faceId),
      newEmpty(),
      nkRefTy.newTree(ty),
    )
  typeSection.add(def)
...
let moduleText = renderTree(module, {renderIds, renderSyms, renderIr, renderNonExportedFields, renderExpandUsing})
