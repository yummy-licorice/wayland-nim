...
proc `$`(msg: Message): string =
  result = "$1 $2 ($3 $4)" % [
      msg.hdr.oid.int.toHex(8),
      msg.buf[1].toHex(8),
      msg.hdr.size.toHex(4),
...
proc send(client: Client; msg: Message) {.asyncio.} =
  ...
  doAssert client.sock.write(msg.buf[0].addr, msg.hdr.size.int) == msg.hdr.size.int
...
proc dispatch*(client: Client) {.asyncio.} =
  ...
  while client.alive:
    ...
    if msg.hdr.size.int < 8:
      ...
      n.inc client.sock.read(msg.buf[2].addr, msg.hdr.size.int - 8)
      if n != msg.hdr.size.int: raise newException(IOError, "Invalid read of Wayland socket. Read " & $n & " bytes of " & $msg.hdr.size)
